...
export function loadConfig(cliOptions = {}) {
  ...
  const envSkipSchema = process.env.FIRESTORE_SKIP_SCHEMA;
  const skipSchema = Boolean(
    firstDefined(
      cliOptions.skipSchema === true ? true : cliOptions.skipSchema === false ? false : undefined,
      envSkipSchema === "1" || envSkipSchema === "true"
        ? true
        : envSkipSchema === "0" || envSkipSchema === "false"
        ? false
        : undefined,
  ...
  const snapshotsDir = path.resolve(
    ...
    firstDefined(
      ...
      resolveFromEnvMap(envMap, "snapshotsDir"),
      file.snapshotsDir,
